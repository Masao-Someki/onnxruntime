...
template <typename T>
__global__ void LegacyRelShiftAddFloat2(const int pos_sequence_length, T* matrix_ac, T* matrix_bd, float alpha) {
  // matrix_bd: BxNxPSxS
  // matrix_ac: BxNxSxS
  ...
  if (i < sequence_length) {
    ...
    if (i < sequence_idx + 1) {
      matrix_ac[ac_offset + i].x = matrix_ac[ac_offset + i].x + matrix_bd[bd_offset + bd_idx + i].x;
      matrix_ac[ac_offset + i].y = matrix_ac[ac_offset + i].y + matrix_bd[bd_offset + bd_idx + i].y;
    } else if (i > sequence_length + 1) {
      matrix_ac[ac_offset + i].x = matrix_ac[ac_offset + i].x + matrix_bd[bd_offset + bd_idx + i - 1].x;
      matrix_ac[ac_offset + i].y = matrix_ac[ac_offset + i].y + matrix_bd[bd_offset + bd_idx + i - 1].y;
    }
    matrix_ac[ac_offset + i].x = matrix_ac[ac_offset + i].x * alpha;
    matrix_ac[ac_offset + i].y = matrix_ac[ac_offset + i].y * alpha;
  }
}

template <typename T>
__global__ void LegacyRelShiftAddHalf2(const int pos_sequence_length, T* matrix_ac, T* matrix_bd, float alpha) {
  // matrix_bd: BxNxPSxS
  // matrix_ac: BxNxSxS
  ...
  if (i < sequence_length) {
    ...
    matrix_ac[ac_offset + i].x = matrix_ac[ac_offset + i].x * __float2half(alpha);
    matrix_ac[ac_offset + i].y = matrix_ac[ac_offset + i].y * __float2half(alpha);
  }
}

template <typename T>
__global__ void LegacyRelShiftAddFloat(const int pos_sequence_length, T* matrix_ac, T* matrix_bd, float alpha) {
  // matrix_bd: BxNxPSxS
  // matrix_ac: BxNxSxS
  ...
  if (i < sequence_length) {
    int bd_idx = (sequence_length - 1) + (sequence_idx * sequence_length) - sequence_idx;
    if (i < sequence_idx + 1) {
      matrix_ac[ac_offset + i] = matrix_ac[ac_offset + i] + matrix_bd[bd_offset + bd_idx + i];
    } else if (i > sequence_length + 1) {
      matrix_ac[ac_offset + i] = matrix_ac[ac_offset + i] + matrix_bd[bd_offset + bd_idx + i - 1];
    }
    matrix_ac[ac_offset + i] = matrix_ac[ac_offset + i] * alpha;
  }
}

template <typename T>
__global__ void LegacyRelShiftAddHalf(const int pos_sequence_length, T* matrix_ac, T* matrix_bd, float alpha) {
  // matrix_bd: BxNxPSxS
  // matrix_ac: BxNxSxS
  ...
  if (i < sequence_length) {
    ...
    matrix_ac[ac_offset + i] = matrix_ac[ac_offset + i] * __float2half(alpha);
  }
}

template <typename T>
__global__ void LatestRelShiftAddFloat2(const int pos_sequence_length, T* matrix_ac, T* matrix_bd, float alpha) {
  // matrix_bd: BxNxPSxS
  // matrix_ac: BxNxSxS
  ...
  if (i < sequence_length) {
    ...
    matrix_ac[ac_offset + i].x = matrix_ac[ac_offset + i].x + matrix_bd[bd_offset + bd_idx].x;
    matrix_ac[ac_offset + i].y = matrix_ac[ac_offset + i].y + matrix_bd[bd_offset + bd_idx].y;
    matrix_ac[ac_offset + i].x = matrix_ac[ac_offset + i].x * alpha;
    matrix_ac[ac_offset + i].y = matrix_ac[ac_offset + i].y * alpha;
  }
}
...
template <typename T>
__global__ void LatestRelShiftAddHalf2(const int pos_sequence_length, T* matrix_ac, T* matrix_bd, float alpha) {
  // matrix_bd: BxNxPSxS
  // matrix_ac: BxNxSxS
...
template <typename T>
__global__ void LatestRelShiftAddFloat(const int pos_sequence_length, T* matrix_ac, T* matrix_bd, float alpha) {
  // matrix_bd: BxNxPSxS
  // matrix_ac: BxNxSxS
  ...
  if (i < sequence_length) {
    const int bd_idx = sequence_length - 1 - sequence_idx + i;
    matrix_ac[ac_offset + i] = matrix_ac[ac_offset + i] + matrix_bd[bd_offset + bd_idx];
    matrix_ac[ac_offset + i] = matrix_ac[ac_offset + i] * alpha;
  }
}

template <typename T>
__global__ void LatestRelShiftAddHalf(const int pos_sequence_length, T* matrix_ac, T* matrix_bd, float alpha) {
  // matrix_bd: BxNxPSxS
  // matrix_ac: BxNxSxS
...
bool LaunchRelShiftAdd(cudaStream_t stream,
                       ...
                       float* matrix_ac, float* matrix_bd, float alpha) {
  // TODO : Support head_size > 1024
  const dim3 grid(sequence_length, batch_size, 1);
  ...
  if (0 == (sequence_length & 1)) {
    ...
    if (sequence_length2 == pos_sequence_length2) {
      LegacyRelShiftAddFloat2<float2><<<grid, block, 0, stream>>>(sequence_length2, matrix_ac2, matrix_bd2, alpha);
    } else {
      LatestRelShiftAddFloat2<float2><<<grid, block, 0, stream>>>(sequence_length2, matrix_ac2, matrix_bd2, alpha);
    }
  } else {
    const dim3 block(sequence_length, num_heads, 1);
    if (sequence_length == pos_sequence_length) {
      LegacyRelShiftAddFloat<float><<<grid, block, 0, stream>>>(sequence_length, matrix_ac, matrix_bd, alpha);
    } else {
      LatestRelShiftAddFloat<float><<<grid, block, 0, stream>>>(sequence_length, matrix_ac, matrix_bd, alpha);
    }
  }
  return CUDA_CALL(cudaPeekAtLastError());
}
...
bool LaunchRelShiftAdd(cudaStream_t stream,
                       const int sequence_length, const int pos_sequence_length, const int batch_size, const int num_heads,
                       half* matrix_ac, half* matrix_bd, float alpha) {
  // TODO : Support head_size > 1024
  const dim3 grid(sequence_length, batch_size, 1);
  if (0 == (sequence_length % 4)) {
    ...
    if (sequence_length2 == pos_sequence_length2) {
      LegacyRelShiftAddFloat2<float2><<<grid, block, 0, stream>>>(
        sequence_length2, matrix_ac2, matrix_bd2, alpha);
    } else {
      LatestRelShiftAddFloat2<float2><<<grid, block, 0, stream>>>(
        sequence_length2, matrix_ac2, matrix_bd2, alpha);
    }
  } else if (0 == (sequence_length & 1)) {
    const int sequence_length2 = sequence_length / 2;
    const int pos_sequence_length2 = pos_sequence_length / 2;
    ...
    const dim3 block(sequence_length, num_heads, 1);
    if (sequence_length == pos_sequence_length) {
      LegacyRelShiftAddHalf<half><<<grid, block, 0, stream>>>(sequence_length, matrix_ac, matrix_bd, alpha);
    } else {
      LatestRelShiftAddHalf<half><<<grid, block, 0, stream>>>(sequence_length, matrix_ac, matrix_bd, alpha);
    }
  }
  return CUDA_CALL(cudaPeekAtLastError());
}
