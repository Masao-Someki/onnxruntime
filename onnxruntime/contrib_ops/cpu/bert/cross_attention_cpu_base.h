...
#include "cross_attention_base.h"
...
class CrossAttentionCPUBase : public CrossAttentionBase {
 protected:
  CrossAttentionCPUBase(const OpKernelInfo& info) : CrossAttentionBase(info) {}

  template <typename T>
  Status ApplyCrossAttention(const T* Q,                // Q data. Its size is BxNxSxH
                             ...
                             int kv_sequence_length,    // sequence length of key
    ...
    size_t attention_probs_bytes = SafeInt<size_t>(batch_size) * num_heads_ * sequence_length * kv_sequence_length * sizeof(T);
    auto attention_probs = allocator->Alloc(attention_probs_bytes);
    BufferUniquePtr scratch_buffer(attention_probs, BufferDeleter(allocator));

    void* mask_data = nullptr;
    if (mask_index != nullptr) {
      size_t mask_data_bytes = SafeInt<size_t>(batch_size) * sequence_length * kv_sequence_length * sizeof(T);
    ...
    auto out_tmp_data =
        allocator->Alloc(SafeInt<size_t>(batch_size) * num_heads_ * kv_sequence_length * v_head_size * sizeof(T));
    ...
    ComputeVxAttentionScore(output->template MutableData<T>(), static_cast<T*>(out_tmp_data), static_cast<T*>(attention_probs), V,
                            batch_size, sequence_length, kv_sequence_length, v_head_size, v_hidden_size, tp);
 ...
 private:
  ...
  void ComputeAttentionProbs(T* attention_probs,                                         // output buffer for the attention probs. Its size is BxNxSxS*
    ...
    const size_t k_input_chunk_length = static_cast<size_t>(kv_sequence_length) * head_size;  // S x H

    {
      ...
      // The cost of Gemm
      const double cost = static_cast<double>(head_size) * sequence_length * kv_sequence_length;

      ThreadPool::TryParallelFor(tp, loop_len, cost, [&](std::ptrdiff_t begin, std::ptrdiff_t end) {
        for (std::ptrdiff_t i = begin; i != end; ++i) {
          const int batch_index = static_cast<int>(i) / num_heads_;
          const int output_offset = static_cast<int>(i) * sequence_length * kv_sequence_length;
          const int mask_offset = batch_index * sequence_length * kv_sequence_length;
          T* output = attention_probs + output_offset;

          // Broadcast mask data: (Bx)SxS* -> (BxNx)SxS*
          if (mask_data != nullptr) {
            memcpy(output, mask_data + mask_offset, sequence_length * kv_sequence_length * sizeof(T));
          }

          const T* k = K + k_input_chunk_length * i;
          // Compute Q*K' + AttentionMask
          //                     original                 transposed             each iteration
          // A: Q                (B x N x) S x H          (B x N x) S x H        S x H
          // B: K'               (B x N x) S* x H         (B x N x) H x S*       H x S*
          // C: attention_probs  (B x N x) S x S*         (B x N x) S x S*       S x S*
          math::Gemm<T, ThreadPool>(CblasNoTrans, CblasTrans, sequence_length, kv_sequence_length, head_size, alpha,
    ...
    //  attention_probs(B, N, S, S*) = Softmax(attention_probs)
    {
      ...
      const int D = kv_sequence_length;
  ...
  void ComputeVxAttentionScore(T* output,                                                // buffer for the result with size BxSxNxH
    ...
    const size_t v_input_chunk_length = static_cast<size_t>(kv_sequence_length * head_size);  // S x H

    const double cost =
        static_cast<double>(sequence_length) * static_cast<double>(head_size) * static_cast<double>(kv_sequence_length);

    ThreadPool::TryParallelFor(tp, batch_size * num_heads_, cost, [&](std::ptrdiff_t begin, std::ptrdiff_t end) {
      for (std::ptrdiff_t i = begin; i != end; ++i) {
        const T* v = V + v_input_chunk_length * i;

        T* current_tmp_data = reinterpret_cast<T*>(tmp_buffer) + v_input_chunk_length * i;
        math::MatMul<T>(sequence_length, head_size, kv_sequence_length,
                        attention_probs + sequence_length * kv_sequence_length * i,
